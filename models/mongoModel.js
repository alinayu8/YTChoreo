 ...
 */
var connection_string = 'mongodb://user:password@ds259325.mlab.com:59325/m2cproject';
...
exports.retrieveAll = function(collection, callback) { //is this right
  /*
   * The find sets up the cursor which you can iterate over and each
   * iteration does the actual retrieve. toArray asynchronously retrieves the
   * whole result set and returns an array.
 ...
 */
 ...
 exports.remove = function(collection, filter, callback) {
    mongoDB
        .collection(collection)     // The collection to update
        .deleteMany(                // Use updateOne to only update 1 document
          ...
          function(err, status) {   // Callback upon error or success
            if (err) doError(err);
            callback('Deleted ' + filter.name + ' if they existed'); //?????
        });
 }
